...
test.serial('Token Needs Refresh', async t => {
    const stringFieild = '{"value": "token-value", "timestamp": 1598512936776}'
    const tokenNeedsRefresh = needsRefresh(JSON.parse(stringFieild).timestamp, 3600)
    t.is(tokenNeedsRefresh, true);
});

test.serial('Token Does Not Need Refresh', async t => {
    const stringFieild = `{"value": "token-value", "timestamp": ${Date.now()}}`
    const tokenNeedsRefresh = needsRefresh(JSON.parse(stringFieild).timestamp, 3600)
    t.is(tokenNeedsRefresh, false);
});

test.serial('Refresh access token', async t => {
    const db = getDb('aw-dms-demo')
